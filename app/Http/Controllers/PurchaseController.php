...
class PurchaseController extends Controller
{
    public function index()
    {
        ...
        if($role->hasPermissionTo('purchases-index')){            
           ...
           $purchase=Purchase::with('Warehouse','supplier','material','shipment')->get();
      // dd($purchase);
            return view('purchase.index', compact('lims_purchase_list', 'lims_account_list', 'all_permission', 'lims_pos_setting_data','purchase'));
        }
        else
    ...
    public function create()
    {
      ...
      $purchase=PurchaseCategory::where('parent_id',NULL)->get();
        ...
        $role = Roles::where('name','Store')->first();

       $shipmentagent=ShipmentAgent::all();

            

            return view('purchase.create', compact('lims_supplier_list', 'lims_warehouse_list', 'lims_tax_list', 'lims_product_list_without_variant', 'lims_product_list_with_variant','shipmentagent','brand','purchase'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function get_raw_matetial_product(Request $request)
    {
             return RawMaterial::where('id',$request->id)->first();   
    }
    ...
    public function purchasecategory(Request $request){

        $categories=PurchaseCategory::where('parent_id',$request->id)->get();
        return $categories;

    }

     public function purchaserawmaterial()
        {
        $role = Role::find(Auth::user()->role_id);
        if($role->hasPermissionTo('products-index')){            
            ...
            $rawmaterial=RawMaterial::with('warehouse','purchasecategory','brand','unit','purchasesubcategory')->get();

         //
    //    dd($rawmaterial);


            return view('purchase.rawmateriallist', compact('all_permission','rawmaterial'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function newrawmaterial(){
        ...
        $brand=Brand::all();
        $unit=Unit::all();
        return view('purchase.addnewrawmaterial',compact('warehouse','purchase','brand','unit'));
    }


    public function save_raw_material(Request $request){

     //   dd($request);
        RawMaterial::insert([

        'warehouse_id'=>$request->store_id,
        'purchasecategory_id'=>$request->purchase_category,
        'subcategory_id'=>$request->subcategory,
        'product_name'=>$request->productname,
        'brand_id'=>$request->brand,
        'product_cost'=>$request->product_cost,
        'productunit_id'=>$request->product_unit,
    ...
    public function purchasesave(Request $request){
        if ($request->hasFile('document')) 
                    ...
                    if(!$request->file('document')->move($destinationPath,$fileName))
                    {
                        throw new \Exception("Failed Upload");                    
                    }

                    $thumbnail = asset("/public/images/images/")."/".$fileName;
    ...
    //   dd($request);
        ...
        foreach ($request->product_name as $key => $product) {
           ...
           // dd($quantity);
            ...
            $discount = isset($request->discount[$key])?$request->discount[$key]:0;
            $total = $request->quantity[$key]*$request->product_cost[$key]; //$request->total[$key];
        ...
        //    dd($product_id);
            Purchase::insert([
                ...
                'agent_id'=>$request->shipmentagentname,
                'shipping_cost'=>$request->shipping_cost,
                'note'=>$request->note
            ]);
            //dd($request);
...
return redirect()->back();
    ...
    public function get_rawmaterial_list(Request $request)
    {
        $val= $request->val;
       $data= RawMaterial::Where('product_name', 'like', '%' . $val . '%')->get(); 
       //dd($data);
       return $data;
    }
