            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header d-flex align-items-center">
                        <h4>{{trans('file.Production List')}}</h4>
                    </div>
                    <div class="card-body">
                    ...
                    @php $allheads = DB::table('production_subcategories')->where('category_id',$cat->id)->where('parent_id',NULL)->select('id','name')->get(); @endphp
                        ...
                        <div class="tab-pane fade show <?php if($i==0) echo 'active' ?>" id="nav-{{$name}}" role="tabpanel" aria-labelledby="nav-{{$name}}-tab">
                              ...
                              @endphp
                               ...
                               @php
                                ...
                                @php $subcat = DB::table('production_subcategories')->where('category_id',$cat->id)->where('id',$sub)->first(); @endphp
                                    ...
                                    @if($subcat->name=='weight' || $subcat->name=='Weight')

                                        @php $weight = $weight + $val; @endphp

                                    @endif

                                    @if($subcat->name=='Boxes' || $subcat->name=='boxes')

                                        @php $boxes = $boxes + $val; @endphp

                                    @endif

                                     @if($subcat->name=='Pieces' || $subcat->name=='pieces')

                                        @php $isolation_peices = $isolation_peices + $val; @endphp
                                  ...
                                  <td>{{$val}}</td>                                  
                                   ...
                                   <td>@if($pro->status==0){{'Avaiable in Production'}} @else {{'Send To Store '}} @endif</td>
                                   <td>
                            ...
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                <li>
                                    <a href="{{route('ProductionApproved',['id'=>$pro->id])}}" type="button" data-id="21" class="open-EditCategoryDialog btn btn-link" ><i class="fa fa-edit"></i> Send to Store</a>
                                </li>
                                 <li>
                               ...
                               @if($cat->name=='Fabric' || $cat->name=='fabric')
                               ...
                               @if($cat->name=='Masks' || $cat->name=='masks' || $cat->name=='Mask' || $cat->name=='mask')
                               ...
                               @if($cat->name=='Isolation Gown' || $cat->name=='isolation gown')

                                <tr>
                                    <td></td>
                                    <td></td>
                                    <td></td>
                                <td><strong>Total : {{$isolation_peices}}</strong></td>
                                    <td></td>
                                    <td></td>
